...
### **Section:** 02G
---
## 🧬 Genetic Algorithm Bit Pattern Optimizer
...
if st.button("🚀 Run Genetic Algorithm"):
    ...
    st.success(f"✅ Optimal Fitness Achieved: **{best_score}**")
    st.text(f"Best Individual (Chromosome): {''.join(map(str, best_individual))}")

    # ------------------------------
    # VISUALIZATION USING STREAMLIT
    # ------------------------------
    df = pd.DataFrame({"Best Fitness": best_fitness_values})
    st.line_chart(df)
...
# FOOTER
# ------------------------------
st.caption("Developed for BSD3513 – Introduction to Artificial Intelligence | Guided by Dr. Ku Muhammad Na’im Ku Khalif")
